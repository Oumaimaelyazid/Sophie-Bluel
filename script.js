function showWork(el) {
  //fonction crée  une balise <figure> contenant l'image et le titre de chaque travail.

  const work = `<figure data-category="${el.categoryId}" data-id="${el.id}">
  ...
  const content = `<figure data-id="${el.id}">
            <img src="${el.imageUrl}" alt="${el.title}">
  ...
  document
    .querySelectorAll(`.btn-del-icon[data-id="${el.id}"]`)
    .forEach((button) => {
...
//Modale permettant d'uploader de nouveaux médias
 const addWork = async() => {
  try {
    ...
    if (
      !picture ||
      !["image/jpeg", "image/png", "image/jpg"].includes(picture.type)
    ) {
      return alert(
        "L'image n'est pas sélectionnée ou son format est incorrect."
      );
    }
    if (picture.size > 4*1024 * 1024) {
        return alert(
            "L'image n'est pas sélectionnée depasse la taille autoriseé"
...
function filterWorks(category) {
  for (const figure of document
    ...
    figure.style.display =
      category == figure.dataset.category || category == 0 ? "block" : "none"; //Afficher les catégories dans deux parties différentes du DOM : sous forme de boutons pour filtrer les travaux et sous forme d'options dans un menu déroulant.
  }
}
...
async function showCategories() {
  try {
    ...
    document.querySelector(".categories-addwork").appendChild(document.createElement("option"))
    for (const el of data) {
      const liBtn = `<li>
      ...
      document
        ...
        .insertAdjacentHTML("beforeend", liBtn);

      document
        .querySelector(".categories-addwork")
        .insertAdjacentHTML(
          "beforeend",
          `<option value="${el.id}">${el.name}</option>`
        ); 
        //ajouter dynamiquement des options à ce menu déroulant
    }
...
if (sessionStorage.getItem("token") !== null) {
  ...
  uploadWorkForm.addEventListener("change", (e) => {
    ...
    uploadWorkImg.insertAdjacentHTML(
      "beforeend",
  ...
  document.getElementById("addwork-form").addEventListener("input", () => {
    document.querySelector(".btn-submit-work").style.backgroundColor =
      document.querySelector(".title-addwork").value.length > 0 &&
      document.querySelector(".img-addwork").value.length > 0 && document.querySelector(".categories-addwork").value.length > 0  ? ""  : "rgb(167, 167, 167)";
